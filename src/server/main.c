...
void handle_new_connection(sock_t serverfd, Pollfds* sockets, Games* games)
{
    ...
    {
        char buffer[8] = { 0 };
        send(connfd, buffer, pack(buffer, "ll", 8, (uint32_t)SERVER_FULL), 0);
        socket_close(connfd);
        return;
    }
...
void handle_client_msg(Pollfds* sockets, Games* games, int index)
{
    ...
    if (revents & POLLIN)
    {
        ...
        debug_buffer_print(buffer, count);
...
int main(void)
{
    ...
    for (;;)
    {
        int ready_to_read_count = socket_poll(sockets.fds, sockets.count, 4);
